...
```bash
docker run --rm --platform=linux/amd64 ghcr.io/nickeyfff/tdx2db:latest -h
```
...
docker run --rm --platform=linux/amd64 \
  -v "$(pwd)":/data \
  ghcr.io/nickeyfff/tdx2db:latest \
  run_csindex_update
...
# 查看所有支持的命令
docker run --rm --platform=linux/amd64 ghcr.io/nickeyfff/tdx2db:latest help
```
